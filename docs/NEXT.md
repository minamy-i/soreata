...
- 行き先の呼び方の見直し（2026-07-13）。「ホーム」という言葉自体の分かりやすさ（「ホームへ戻る」の文言含む）、「チームルーム」「チームの部屋」等への改名を検討する。マイページ（`/account`）も「ホーム」っぽく感じるという指摘あり（2026-07-13）。決定経緯 → `docs/NOTES.md`「『移動』（チーム一覧）と『戻る』（設定・記録詳細）は同じ行き先でも動詞を揃えない。行き先自体の呼び方は保留」参照。着手は後でよい
