...
- [ ] チームページ改修：チーム設定ページ本体（`app/home/[team_id]/settings`）
  - 今は`persons`参照の旧設計のまま・team_id未対応（次回着手）
  - チーム削除（2パターン。確認ダイアログの文言・削除範囲は検討中→着手前に相談）
  - 外部ツールへの投稿設定（Webhook URL・委譲する協力者の指定）
  - 当人権限（owner）移譲の指名（UI詳細は検討中→着手前に相談）
  - 協力者を招待する・招待中一覧＋取り消し（「招待フロー実装」タスクと合わせて実装）
  - Webhook管理を委譲された協力者＝連携設定のみ表示（他セクション非表示）
  - 記録詳細ページにWebhook投稿ボタンを追加（チームにURL設定済みの場合のみ表示。コピー機能と同じテキストをSlack/Discordへ送信）
...
- [ ] `app/login/page.tsx`：`persons`参照が残っている（ログイン済み・`next`パラメータ無しで`/login`を開いた場合の分岐）。チームページ改修中に発見（2026-07-05）
