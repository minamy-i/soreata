...
## 進行中・未コミット（2026-07-10時点でworking treeに残っている変更）

- [x] 「チーム一覧」→「マイページ」に改名（ナビラベル・`/account`のページ見出し・docs）。実装済み・未コミット
- [x] `.page-title`・`.team-name-link`・`.team-name-tag`の見た目を確定（白背景・角を4pxだけ丸めた四角で統一）。実装済み・未コミット（決定経緯 → `docs/NOTES.md`「team-name-tagをAI分解画面からナビバーへ移動。3箇所の角を4pxで統一」参照）
- [x] `team-name-tag`をAI分解画面からナビバー（マイページ/ログインリンクの手前）へ移動。`lib/use-active-team.ts`で判定ロジックを共通化。実装済み・未コミット
- [x] 鉛筆の編集アイコンを絵文字からSVG（`app/components/PencilIcon.tsx`）に変更。実装済み・未コミット
...
- チーム0件のまま放置されるアカウントへの案内（2026-07-08）。チーム0では登録している意味がないため、`/account`（マイページ）側で何か案内を検討する。着手は後でよい
