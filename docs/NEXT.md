...
- [x] グローバルナビの未ログイン時ラベルを「ログイン」→「Googleでログイン」に修正（2026-07-08）
  - きっかけ：分解前の`.login-note`削除（直前のpush）でログインの入口がナビだけになり、「分解後じゃ遅い」「Googleだとすぐ分かる方がいい」との指摘
  - `docs/SPEC.md`のグローバルナビ表には元々「Googleでログイン」と記載済みで、`app/components/GlobalNav.tsx`側だけが古い「ログイン」のままだった（新規決定ではなくSPECとの乖離修正）。SPEC.md側の変更は不要
  - フル表示・スマホ短縮表示とも同じ文言（ナビの項目数が少なく短縮不要のため分けない）
  - 併せて確認・変更なしと決定：ログイン画面の「戻る」はブラウザ「←」想定で問題なし／Googleのアカウント選択画面はsoreata固有でなくGoogle標準の挙動（`prompt=select_account`未指定のため複数ログイン時のみ自動表示）
- [x] AI分解画面の結果カード「それ以外（A以外）」向け案内を整理（2026-07-08）
  - きっかけ：上記のナビ文言修正でナビに常時「Googleでログイン」が出るようになり、分解後の同じボタンが重複入口になっていた（開発者・Claude双方で認識が食い違っていたのも一因）
  - 検討の途中で気づいた問題：分解後の結果はその場限りで保存されない（未ログイン状態では登録・チーム作成をしても`/account`へ遷移するだけで、今の結果自体は残らない）。「今すぐログインすれば今の結果が保存できる」ように読める文言は誤解を招くため不採用
  - 最終形：「未登録の方はコピーを」という案内文・専用ボタンは撤去（コピーボタンは元々誰でも使える汎用ボタンで、対象を限定する文言は不要と判断）。案内文は「次回以降の結果は、登録後のチーム作成・所属により保存できます。」の1文のみに整理
  - `app/page.tsx`の`signIn`関数はこの画面で未使用になったため削除（ナビ側の`/login`遷移で完結）
  - `docs/SPEC.md`「結果エリア（コピーボタン横）」の記載も1文構成・コピー案内撤去に更新
- [ ] NEXT.mdの棚卸し（次回の区切りで実施）
  - 完了項目が2026-07-04以降ずっと削除されずに溜まっている。本来は完了後に削除しNOTES行きの要否だけ判断する運用（CLAUDE.mdルール）のはずが、pushのたびに残していた
  - 対応：完了済み`[x]`項目を精査し、蒸し返し・連動・不採用案のいずれかに該当するものだけNOTES.mdへ移し、それ以外は削除する
